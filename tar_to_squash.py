...
for line in file_list_fd:
    ...
    if len(spl) > 2:
        ...
        tarball_no_sfx = '.'.join(spl[0:-1])
    # untar/decompress
    untar_args = ['tar', '-C', squash_dir, '-xzf', tarball]
    print('Running: ' + ' '.join(untar_args))
    subprocess.run(untar_args, check=True)
    # squash
    squash_args = ['mksquashfs', tarball_no_sfx, tarball_no_sfx + '.sqfs', '-no-duplicates']
    print('Running: ' + ' '.join(squash_args))
    subprocess.run(squash_args, check=True)
    if delete_tar:
        os.remove(tarball)
        os.remove(tarball_no_sfx)
file_list_fd.close()
